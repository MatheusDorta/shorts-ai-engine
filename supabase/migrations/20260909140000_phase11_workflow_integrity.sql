-- Phase 1.1: ownership integrity, atomic approval/scheduling, publish lock.
-- This file is a migration definition only; it is not applied to production by this change.

-- Ownership: content.source_id and content.affiliate_link_id must belong to the same user.
CREATE OR REPLACE FUNCTION public.assert_content_related_owner() RETURNS trigger
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  IF NEW.source_id IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM public.sources s WHERE s.id = NEW.source_id AND s.user_id = NEW.user_id
  ) THEN
    RAISE EXCEPTION 'source must belong to the same user';
  END IF;
  IF NEW.affiliate_link_id IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM public.affiliate_links a WHERE a.id = NEW.affiliate_link_id AND a.user_id = NEW.user_id
  ) THEN
    RAISE EXCEPTION 'affiliate link must belong to the same user';
  END IF;
  RETURN NEW;
END; $$;
REVOKE ALL ON FUNCTION public.assert_content_related_owner() FROM PUBLIC, anon, authenticated;
DROP TRIGGER IF EXISTS content_related_owner ON public.content;
CREATE TRIGGER content_related_owner
  BEFORE INSERT OR UPDATE ON public.content
  FOR EACH ROW EXECUTE FUNCTION public.assert_content_related_owner();

-- Ownership: content_platforms.content_id must belong to the same user.
DROP TRIGGER IF EXISTS content_platforms_content_owner ON public.content_platforms;
CREATE TRIGGER content_platforms_content_owner
  BEFORE INSERT OR UPDATE ON public.content_platforms
  FOR EACH ROW EXECUTE FUNCTION public.assert_child_content_owner();

-- Published is only valid after a real future platform publishing response.
CREATE OR REPLACE FUNCTION public.prevent_manual_published() RETURNS trigger
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  IF auth.uid() IS NOT NULL AND NEW.status = 'published' AND (
    TG_OP = 'INSERT' OR OLD.status IS DISTINCT FROM 'published'
  ) THEN
    RAISE EXCEPTION 'published status can only be set after a real platform publishing response';
  END IF;
  RETURN NEW;
END; $$;
REVOKE ALL ON FUNCTION public.prevent_manual_published() FROM PUBLIC, anon, authenticated;
DROP TRIGGER IF EXISTS content_prevent_manual_published ON public.content;
CREATE TRIGGER content_prevent_manual_published
  BEFORE INSERT OR UPDATE OF status ON public.content
  FOR EACH ROW EXECUTE FUNCTION public.prevent_manual_published();

CREATE OR REPLACE FUNCTION public.prevent_manual_job_published() RETURNS trigger
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  IF auth.uid() IS NOT NULL AND NEW.status = 'published' AND (
    TG_OP = 'INSERT' OR OLD.status IS DISTINCT FROM 'published'
  ) THEN
    RAISE EXCEPTION 'published job status can only be set after a real platform publishing response';
  END IF;
  RETURN NEW;
END; $$;
REVOKE ALL ON FUNCTION public.prevent_manual_job_published() FROM PUBLIC, anon, authenticated;
DROP TRIGGER IF EXISTS publishing_jobs_prevent_manual_published ON public.publishing_jobs;
CREATE TRIGGER publishing_jobs_prevent_manual_published
  BEFORE INSERT OR UPDATE OF status ON public.publishing_jobs
  FOR EACH ROW EXECUTE FUNCTION public.prevent_manual_job_published();
...
-- Atomic local scheduling. Does not publish to YouTube or TikTok.
...
BEGIN
  ...
  SET status = 'waiting', result = 'Cancelled locally. Platform not connected.'
  WHERE id = p_job_id
  RETURNING * INTO v_job;
